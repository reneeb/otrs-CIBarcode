...
Defines which config item attribute is encoded in the barcode. Please note that EAN8 and EAN13 barcodes can handly only numeric attributes that are not longer than the number in the name (EAN8: 8-digit numbers, EAN13, 13-digit numbers).
